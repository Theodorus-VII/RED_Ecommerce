...
public class ProductService : IProductService
{
    ...
    {
        Product? match = await _context.Products
            .Include(p => p.Images)
            .FirstOrDefaultAsync(p => p.Id == id && p.Count > -1);
    ...
    public async Task<FilterAttributesResponse> GetProductByFilter(FilterAttributes filterAttributes, int start, int maxSize, bool isAdmin)
    {
        try
        {
            ...
            // I changed the if-else statements to a switch case. Added a a few other sorting types and it got difficult to look at.
            switch (type)
            {
                case SortType.PRICE_ASCENDING:
                    finalProducts = finalProducts.OrderBy(product => product.Price).ToList();
                    break;

                case SortType.PRICE_DESCENDING:
                    finalProducts = finalProducts.OrderByDescending(product => product.Price).ToList();
                    break;
